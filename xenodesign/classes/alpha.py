...
This module is the CANONICAL home of the α-case design logic (migrated from
``scripts/design_alpha.py`` in T5 of the multi-class framework plan, behaviour-
preserving). ``scripts/design_alpha.py`` is now a thin shim that re-exports every
public name from here and keeps its ``run_alpha_design`` / ``_parse_args`` / CLI.

The :class:`Alpha` adapter at the bottom implements the
:class:`xenodesign.classes.base.BinderClass` protocol by delegating to the same
helpers the legacy driver used, so the dispatcher (``xenodesign/dispatch.py``) can
wire the α loop without knowing the binder chemistry.
...
# imports to call time, so importing this module loads neither torch nor carbonara. These are
# module-level so the mixed dispatcher and tests can rebind/monkeypatch them.
from xenodesign.carbonara_backend import carbonara_design_fn
from xenodesign.sequence_update import _ligandmpnn_design_fn

# NOTE: SeedSpec is imported lazily inside Alpha.seed (not at module top) to avoid a circular
# import — base.py re-points its registry at this module (`from ...alpha import Alpha`), so a
# top-level `from ...base import SeedSpec` here would deadlock when alpha is imported first.
...
def _make_referee_fn(loop_dir, esm_judge=None, roles=None):
    ...
    def _score(step, idx):
        ...
        if cif is not None:
            try:
                scored_l_seq = binder_seq_from_cif(cif, binder_chain)
            except Exception:
                scored_l_seq = None
...
def _assemble_alpha_result(history, referee_scores, panel_result, case, *,
    ...
    for i, (step, ref) in enumerate(zip(history, referee_scores)):
        try:
            l_seq = binder_seq_from_cif(_best_cif_path(loop_dir / f"iter_{i:03d}"),
                                        _RUN_BINDER_CHAIN)
    ...
    if objective in ("mixed", "ipsae"):
        ...
        for i in range(len(history)):
            if panel_result.vetoed[i]:
                continue
            try:
                cif = _best_cif_path(loop_dir / f"iter_{i:03d}")
                if objective == "ipsae":
...
# ── Main driver (kept here so the shim's CLI delegates to one place) ────────────
